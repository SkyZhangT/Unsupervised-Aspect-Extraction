...
class W2VEmbReader:
    ...
    def get_emb_matrix_given_vocab(self, vocab, emb_matrix):
        ...
        for word, index in vocab.items():
            try:
                emb_matrix[index].data = torch.Tensor(self.embeddings[word]).data
                counter += 1
            except KeyError:
                pass
        ...
        # L2 normalization
        norm_emb_matrix = emb_matrix / torch.linalg.norm(emb_matrix, dim=-1, keepdims=True)
    ...
    def get_aspect_matrix(self, n_clusters):
        ...
        clusters = torch.tensor(km.cluster_centers_)

        # L2 normalization
        norm_aspect_matrix = clusters / torch.linalg.norm(clusters, dim=-1, keepdims=True)
        return norm_aspect_matrix.type(torch.float32)
